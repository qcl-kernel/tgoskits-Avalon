...
> 交付状态说明（2026-08-24）：本文件记录设计和排查历史；实际完成状态及可用于比赛结论的运行数据，以同目录 `测试报告.md` 为准。
...
奇数轮先运行共享等待基线，偶数轮先运行优化路径。`summarize_rt_multirun.py` 对每个指标同时输出三轮中位数和范围，避免只挑选有利轮次。

**三轮中位数结论**

- 压力 RTT avg 退化 0.13%，p99 改善 0.53%，max 改善 33.95%。
- 压力 RTOS 服务时间 avg 改善 4.37%，p99/max 退化 10.26%/1.31%。
- 压力唤醒迟到 avg/p99/max 改善 16.23%/1.24%/54.96%。
- 压力周期间隔绝对抖动 avg/p99/max 改善 10.68%/8.85%/61.62%。
- 压力请求间隔偏差 p99/max 改善 12.17%/37.43%，三轮均无 deadline miss。
- 空载 RTT avg/p99/max 改善 9.99%/3.68%/4.29%，周期间隔抖动 avg/p99/max 改善 14.42%/29.69%/22.23%。
- 空载唤醒迟到 avg 退化 0.63%，请求间隔偏差 p99/max 退化 3.04%/65.35%。

优化组空载第二轮出现 26.659 ms 唤醒迟到离群值和一次 deadline miss。结论应限定为“压力场景最坏值与周期确定性明显改善，平均 RTT 基本持平，仍存在服务时间尾部和空载离群退化”，不能表述为全面加速或已获得硬实时上界。
