...
| 功能 | 主要文件 | 具体修改 | 设计原因 |
| --- | --- | --- | --- |
| AxVisor 实时配置 | `os/axvisor/src/config.rs`、`main.rs`、`manager.rs`、`platform_irq.rs` | 解析并校验 vCPU/pCPU 绑定和 IRQ 目标；启动时建立 VM、vCPU 与物理 CPU 的稳定映射 | 把实时策略放进显式配置，避免按 Guest 名称或启动顺序猜测 |
| vCPU 等待和唤醒 | `virtualization/axvm/src/runtime/vcpus.rs`、`lifecycle.rs`、`rt_trace.rs`、`vm.rs` | 将业务唤醒收敛到目标 vCPU，维护 pending 状态和生命周期同步；增加低频实时统计 | 减少无关 vCPU 唤醒、锁竞争和压力长尾 |
| AArch64 IRQ 注入 | `virtualization/axvm/src/runtime/aarch64_irq.rs`、`irq/mod.rs`、`host/gic.rs`、`arch/aarch64/mod.rs` | 根据路由选择目标 vCPU，记录 pending IRQ，统一 active/EOI 和重新注入路径 | 正确处理水平触发设备并避免 IRQ 只触发一次 |
| 通用平台 IRQ API | `platforms/ax-plat/src/irq.rs`、`platforms/axplat-dyn/src/irq.rs`、`platforms/somehal/src/irq.rs`、`arch/aarch64/gic/` | 补齐 enable/disable、mask/unmask、affinity 和状态能力；动态平台透传同一语义 | RTOS、Linux 和不同板级共用接口，不在虚拟机层写平台特判 |
| GICv3 路由修复 | `drivers/intc/arm-gic-driver/src/version/v3/mod.rs`、`platforms/somehal/src/arch/aarch64/gic/v3.rs` | 修正目标 CPU/affinity 编程和等待状态 | 保证设备中断稳定送到配置的 pCPU/vCPU |
| VM 构建和设备装载 | `virtualization/axvm/src/vm/prepare/vcpus.rs`、`devices.rs`、`boot/images/mod.rs`、`config.rs` | 校验 CPU/内存/设备配置，按 VM 配置装载镜像、DTB 和设备 | 让 Linux 2-vCPU 与 RTOS 1-vCPU 资源分配可复现 |
...
1. 每个 vCPU 维护独立等待对象和待处理状态。
2. vCPU 无工作时进入自己的等待队列，不与其他 vCPU 共用一次业务唤醒。
3. IRQ 路由确定目标 vCPU 后只唤醒目标，减少 `notify_all` 式惊群。
4. 待注入 IRQ 先进入目标 vCPU 的 pending 路径，vCPU 在进入 Guest 前统一处理。
5. VM 停止、销毁等全局事件仍使用全局生命周期同步，不把局部优化扩展到不适合的状态机。
6. `rt_trace.rs` 只做低频聚合统计，记录 wake-to-run、VM-exit、IRQ dispatch/handler 等指标，避免逐事件日志反过来污染实时性。
...
1. 设备准备阶段从 passthrough 设备和 DTB 中取得 IRQ 资源。
2. VM 启动时注册 IRQ owner、目标 vCPU 和目标 pCPU 亲和性。
3. VM 可运行后才 enable/unmask 对应 IRQ。
4. VM 停止或销毁时先 mask/disable，再解除 owner 和 handler，避免 IRQ 注入已停止 VM。
5. IRQ 到达时检查 VM/vCPU 状态；不可注入时返回明确错误或保持 pending，不根据 Guest 类型猜测处理方式。

这种设计能复用于新的 RTOS 和设备。Guest 之间的差异只体现在 VM 配置、DTB 和 Guest 驱动，不侵入 AxVisor 通用 IRQ 核心。
...
`os/axvisor/configs/board/qemu-aarch64-rt-shared-wait-baseline.toml` 启用旧的共享 vCPU 等待队列和广播唤醒路径，正常板级配置启用按 vCPU 独立等待队列和定向唤醒路径。对应 Cargo feature 名为 `rt-shared-wait-baseline`，仅用于生成实时改造前后的 A/B 对照数据，不是 RTOS、Guest 兼容层或正式部署模式。

`run_axvisor_rt_before_after.sh` 对两种配置使用同一 Linux/RTOS 镜像、VM 拓扑、请求次数和压力进程，分别收集 idle/stress 数据，再由 `summarize_rt_before_after.py` 汇总。这样比较的是 AxVisor 关键路径，而不是不同 Guest 或不同负载。
...
Zephyr 应用和 overlay 位于 `apps/ai-rtos-demo/zephyr/`。官方网络栈负责 socket/TCP/IP；仓库侧 `virtio_mmio_legacy.c` 处理 QEMU legacy virtio 队列、IRQ ACK 和完成队列恢复。设备由 `axvisor-virtio.overlay` 声明，不修改 Zephyr 上游 DTS 或驱动源码。

### 4.5 FreeRTOS 网络 Guest

`NetworkInterface.c` 把 FreeRTOS+TCP 的网络接口契约连接到 `virtio_net.c`。`platform.c` 处理 GIC、timer 和 IRQ 分发；`main.c` 创建网络、AICP 服务和基线任务。AxVisor Guest 构建和独立 QEMU 基线构建使用编译选项区分 GIC owner，避免 Guest 在 AxVisor 下重新初始化宿主管理的 GIC。
...
`scripts/ai-rtos/run_freertos_periodic_baseline.sh` 输出空载和双压力场景的固定样本数、平均值、p99、最大绝对抖动与 deadline miss。报告时应同时保留两个场景的原始 CSV，不能只用“无 deadline miss”替代尾延迟分析。

### 7.20 长稳日志归档必须区分主 QEMU 串口与 Linux 独立串口

长稳运行器为每次执行保存主 QEMU 日志、Linux 串口日志、runner 输出、合并日志与统计摘要。归档逻辑不能仅按同名前缀排序，否则 Linux 串口文件可能被误当作主 QEMU 日志，导致控制事务和实时字段来自不同来源。

`lib/run_artifacts.sh` 通过运行器显式返回的 `log=` 与 `linux_console_log=` 归档两条路径，并拒绝缺失或相同路径。`test_run_artifacts.sh` 覆盖同名前缀、缺少串口和错误排序场景。长稳成功率、RTT、服务时间与 deadline 数据只应从本次运行生成的摘要读取。
...
RT-Thread 横向表使用相邻周期的 `abs(actual_interval - 20 ms)`。固定时间轴绝对偏差继续保留在原始摘要中，用来说明时钟和 tick 偏差，不用于宣称单次最坏调度延迟。
...
### 7.25 实时 A/B 采用多轮交替统计

`rt-shared-wait-baseline` 保留旧共享 wait queue/broadcast wake 行为，`rt-poll-idle` 启用每 vCPU 等待/唤醒与 timer/device poll 推进。比较时应使用同一 Linux 2-vCPU + ArceOS 拓扑、同一请求数与压力配置，并交替执行两种配置，防止 QEMU TCG 热机和宿主调度顺序影响单轮结论。

统计器记录每轮的 RTT、RTOS 服务时间、唤醒迟到、周期抖动、请求间隔、CPU 负载和 deadline miss；报告以本次原始 CSV 和汇总文件为准。当前已执行的运行时回归是 AArch64 `rt-poll-idle-timer-wake`，验证 idle 到 timer wake 的继续执行路径。
...
### 7.28 Zephyr 周期基线的超时配置

Zephyr 周期基线在 QEMU TCG 下会受宿主负载影响。脚本应以样本数、成功 marker 和统计输出作为完成条件，超时仅用于终止失去进展的运行，不应通过减少样本数掩盖失败。每次执行后的原始 CSV 和日志应随结果一并保存。
...
在允许进程监听 loopback 的普通开发环境执行 `scripts/ai-rtos/run_aicp_protocol_reliability.sh 50`。受限沙箱中仍可单独运行不创建监听端点的协议编解码单元测试；完整可靠性结果须以该次运行归档为准。

### 7.30 长稳统计的日志源选择

业务 STATUS、RTOS timing 和 AxVisor trace 分别可能位于不同串口文件。统计器必须显式接收主 QEMU 日志和 Linux 串口日志，分别提取后再生成合并结果；不能把任意同名前缀文件当作完整运行日志。ArceOS 与 RT-Thread 长稳入口复用该归档边界。
...
该规则由隔离检查单元测试覆盖，并可用于每个 Guest 组合的独立运行日志审计。
...
当前实际完成的 QEMU 证据为 Linux–ArceOS 轻量神经网络闭环、Linux–FreeRTOS 轻量神经网络闭环、Linux–ArceOS Rust YOLOv8n + ONNX Runtime CPU 三图闭环，以及 AxVisor `rt-poll-idle-timer-wake` 回归。其余 Guest 组合、长稳、实时 A/B 与原生 RTOS 基线均保留独立构建、启动、采集与汇总入口，运行结果应以对应命令产物为准。
...
scripts/ai-rtos/checks/check_shell_syntax.sh
git diff --check
```
...
5. 闭环日志必须同时具备模型或控制输入、CONTROL 发送、RTOS 控制执行、STATUS 回传和最终成功 marker。
