...
> 交付状态说明（2026-08-24）：本文件保留完整的设计、实现与复现说明。所有“已实测”结论均必须以同目录 `测试报告.md` 中列出的命令和原始日志为准；未在该报告中记录的 StarryOS、YOLOv8、RT-Thread、Zephyr 或长期压力结论仍在验证，不作为本次提交的已完成证据。
...
实时路径最终调整后再次执行 `AICP_FULL_PREPARE_IMAGES=0 scripts/ai-rtos/run_full_qemu_validation.sh smoke`，主机协议预检、第三方源码边界、架构检查和八组 Guest 闭环全部为 `PASS`。该 smoke 使用每组 1 次事务确认兼容性；性能、可靠性和长样本结论仍以上述完整 full、三轮 A/B 和万次长稳为准。
...
对照组启用 `rt-shared-wait-baseline`，恢复旧共享等待队列和广播唤醒；优化组使用原子 vCPU 状态机、每 vCPU 独立等待队列、定向唤醒、每 vCPU pending IRQ 队列、锁外中断注入、Guest timer deadline cache 和通用 IRQ active/EOI 生命周期。两组拓扑均为 Linux 2-vCPU + ArceOS 控制 Guest，每个空载/压力场景执行 300 次事务，压力场景在 Linux 两个 vCPU 上各运行一个压力进程。
...
三轮中位数表明，优化的主要收益集中在压力场景最坏值和周期确定性：RTT 最大值改善 33.95%，唤醒迟到最大值改善 54.96%，周期间隔绝对抖动最大值改善 61.62%，三轮均无 deadline miss。压力 RTT 平均值基本持平，但 RTOS 服务时间 p99/max 分别退化 10.26% 和 1.31%，不能宣称所有指标都改善。

空载中位数多数改善，但优化组第二轮出现 26.659 ms 唤醒迟到离群值和一次 deadline miss，请求间隔偏差最大值的中位数退化 65.35%。QEMU TCG 受宿主操作系统调度和计时噪声影响，因此结果采用三轮中位数和范围，不把单轮最大值解释为硬实时上界。原始轮次、每轮顺序和全部指标保存在实时 A/B 归档的 `multirun.summary.txt` 中。
...
AICP_LINUX_DONE ok=20 failed=0 avg_rtt_ns=236130752 max_rtt_ns=276277008
```
...
| 平均 RTT | 411,610 ns | 345,760 ns | 减少 65,850 ns |
| 最大 RTT | 13,585,000 ns | 6,635,000 ns | 减少 6,950,000 ns |
...
| AxVisor 关键机制实质改造，8 分 | QEMU 已实测 | 每 vCPU 等待队列、定向唤醒、pending IRQ、锁外注入、通用 IRQ 生命周期、GPPT affinity 修复、Guest timer 停止清理 |
| 多核 Linux 配置，4 分 | QEMU 已实测 | Linux 2 vCPU 绑定 pCPU1/pCPU2，512 MiB RAM，独立 virtio-net；RTOS 绑定 pCPU0 |
| 改造前后数据，5 分 | QEMU 已实测 | 三轮交替 A/B；压力 RTT max 改善 33.95%，唤醒迟到 max 改善 54.96%，周期间隔抖动 p99/max 改善 8.85%/61.62%；同时公开压力 RTOS 服务 p99/max、空载请求偏差和离群 deadline miss 等退化项 |
