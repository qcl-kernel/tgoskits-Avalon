...
3. Linux AI Guest 的轻量神经网络推理、跨 Guest TCP/IP 控制下发、ArceOS/FreeRTOS 控制输出和状态回传。

当前完整实跑闭环是 Linux（2 vCPU）与 ArceOS（1 vCPU），以及 Linux（2 vCPU）与 FreeRTOS（1 vCPU）。它不把代码中的 StarryOS、YOLOv8/RKNN、RT-Thread 或 Zephyr 适配入口误表述为同样已经完成的 AxVisor 闭环。
...
当前实跑 smoke 使用 3 次 AI 模式事务，成功标记为：
