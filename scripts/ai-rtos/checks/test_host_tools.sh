...
source_repo="${test_dir}/pinned-source"
source_remote="${test_dir}/pinned-source.git"
source_checkout="${test_dir}/pinned-checkout"
git init -q "${source_repo}"
git -C "${source_repo}" config user.email test@example.invalid
git -C "${source_repo}" config user.name "AICP host-tool test"
...
git clone -q --bare "${source_repo}" "${source_remote}"
third_party_prepare_pinned_git_source \
  "pinned-source" "${source_checkout}" "V1.0.0" "${source_remote}" 1
if [[ "$(git -C "${source_checkout}" describe --exact-match --tags)" != "V1.0.0" ]]; then
  echo "FAIL: 自动准备的第三方源码未停在请求版本" >&2
  exit 1
fi
