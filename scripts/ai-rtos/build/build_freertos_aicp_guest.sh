...
tcp_ref="${FREERTOS_PLUS_TCP_REF:-V4.3.2}"
...
kernel_is_default=0
tcp_is_default=0
[[ -z "${FREERTOS_KERNEL_DIR:-}" ]] && kernel_is_default=1
[[ -z "${FREERTOS_PLUS_TCP_DIR:-}" ]] && tcp_is_default=1

third_party_prepare_pinned_git_source \
  "FreeRTOS-Kernel" "${kernel_dir}" "${kernel_ref}" \
  "https://github.com/FreeRTOS/FreeRTOS-Kernel.git" "${kernel_is_default}"
third_party_prepare_pinned_git_source \
  "FreeRTOS-Plus-TCP" "${tcp_dir}" "${tcp_ref}" \
  "https://github.com/FreeRTOS/FreeRTOS-Plus-TCP.git" "${tcp_is_default}"
