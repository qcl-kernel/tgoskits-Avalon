...
# Archive the log or logs returned by a dual-guest runner. Current AxVisor
# internal-switch runs multiplex host and guest consoles into `log`; older
# usernet runners may additionally return an independent Linux console log.
aicp_archive_dual_guest_logs() {
  ...
  if [[ -n "${linux_console_log}" && ! -f "${linux_console_log}" ]]; then
    echo "ERROR: runner returned an invalid Linux console log path" >&2
    return 1
  fi
  if [[ -n "${linux_console_log}" && "${qemu_log}" == "${linux_console_log}" ]]; then
