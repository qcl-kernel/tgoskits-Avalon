...
# Prepare a TGOSKits-owned disposable checkout at an immutable release ref.
# Callers may only enable automatic cloning for their default tmp/ location;
# an explicit source path is owned by the user and must already be a clean
# checkout at the requested ref.
third_party_prepare_pinned_git_source() {
  local name="$1"
  local source_dir="$2"
  ...
  if [[ ! -e "${source_dir}" ]]; then
    if [[ "${allow_clone}" != "1" ]]; then
      echo "ERROR: ${name} source does not exist: ${source_dir}" >&2
      return 1
    fi
    echo "[ai-rtos] cloning ${name} at ${required_ref}" >&2
    git clone --depth 1 --branch "${required_ref}" "${remote_url}" "${source_dir}"
  fi

  third_party_assert_git_source "${name}" "${source_dir}" "${required_ref}"
}
