...
usage() {
  cat <<'EOF'
Usage:
  scripts/ai-rtos/run_axvisor_aicp_e2e.sh [iterations] [ai|fixed] [boot_timeout_seconds]

Boots AxVisor with the ArceOS AICP RTOS guest, waits for virtio-net/DHCP/AICP
readiness, then runs the Linux-side AICP client through QEMU host forwarding:

  host/Linux client 127.0.0.1:18800 -> QEMU user-net -> RTOS guest 10.0.2.15:8800
EOF
}
...
log_file="${log_dir}/axvisor-aicp-e2e-${stamp}.log"
csv_file="${demo_dir}/build/axvisor_aicp_${mode}_${stamp}.csv"
summary_file="${demo_dir}/build/axvisor_aicp_${mode}_${stamp}.summary.txt"
...
echo "[ai-rtos] Preparing RTOS guest and AICP client binaries..."
"${repo_root}/scripts/ai-rtos/setup_qemu_rtos.sh" arceos-aicp
make -C "${demo_dir}" all
make -C "${demo_dir}" cross-aarch64
...
wait_for_marker "AICP ArceOS RTOS server listening"

echo "[ai-rtos] Running AICP client iterations=${iterations} mode=${mode}"
"${demo_dir}/build/aicp_client" 127.0.0.1 18800 "${iterations}" "${csv_file}" "${mode}"
...
echo "[ai-rtos] PASS: AxVisor AICP e2e complete"
