...
本手册面向赛事评审与复现人员，集中说明系统架构、客户机配置、代码实现、通信协议、智能推理、实时性改造、测试结果、复现步骤、许可证和验收方法。所有结论均以源码、配置、自动化脚本和严格成功标记为依据；尚待开发板验证的内容单独标注。
...
| --- | --- | --- | --- | --- | --- | --- |
| Linux | 2-vCPU 智能计算 Guest | 已实测 | 与四种控制 Guest 的 TCP 均已实测 | C 轻量神经网络控制；Rust YOLOv8n CPU 到 RT-Thread，已实测 | 作为被测多核 Guest | 主线闭环已验证 |
| StarryOS | Linux 替代智能计算 Guest | 已实测 | 与四种控制 Guest 的 TCP 均已实测；UDP 用于故障注入 | 原生 AICP 控制闭环，已实测 | 不作为原生 RTOS 基线 | 加分闭环已验证，保留 Linux 路径 |
| ArceOS | 独立 Rust 控制 Guest；AICP 参考服务端 | 已实测 | virtio-net + TCP，Linux/StarryOS 双 Guest 均已实测 | CONTROL_SET、STATUS、ERROR 闭环已实测 | 用于 AxVisor 实时 A/B 和万次长稳 | 闭环已验证，兼作协议参考实现 |
| RT-Thread | 独立控制 Guest；原生 RTOS 基线 | 已实测 | virtio-net + lwIP + TCP，已实测 | 接收 YOLOv8 输出、执行控制、回传状态，已实测 | 20 ms 周期空载/压力，已实测 | 闭环与基线均已验证 |
| Zephyr | 独立控制 Guest；原生 RTOS 基线 | 已实测 | virtio-net + TCP，Linux/StarryOS 双 Guest 均已实测 | CONTROL_SET、STATUS、ERROR 闭环已实测 | 20 ms 周期空载/压力，已实测 | 闭环与基线均已验证 |
| FreeRTOS | 独立控制 Guest；原生 RTOS 基线 | 已实测 | FreeRTOS+TCP + virtio-net，Linux/StarryOS 双 Guest 均已实测 | CONTROL_SET、STATUS、ERROR 闭环已实测 | 20 ms 周期空载/压力，已实测 | 闭环与基线均已验证 |

已验证范围为：**Linux 和 StarryOS 两种智能计算 Guest 均可与 ArceOS、RT-Thread、Zephyr、FreeRTOS 四种控制 Guest 完成基于 TCP/IP 的双向控制闭环；Linux 主线和 StarryOS 加分路径同时保留。** RT-Thread、Zephyr、FreeRTOS 另外提供同类 20 ms 周期任务的原生 RTOS 空载/压力基线。
...
| 类型 | 位置 | 内容 |
| --- | --- | --- |
| 技术报告与复现手册 | `docs/ai-rtos/完整全流程实现与复现手册.md` | 架构、实现、测试、复现、评分映射、许可证和验收命令 |
...
以上八组日志来自一次统一 `full` 执行。该执行覆盖协议可靠性、AI 模型、实时 A/B 和三种 RTOS 基线等阶段；其后实时路径与 ArceOS socket 超时实现继续发生修改，因此下面的汇总用于说明当次全流程覆盖范围，不作为最终代码头再次整体通过的结论：
...
### 5.2 AxVisor 实时改造前后 A/B

为在同一最新 `dev` 基点、相同镜像、相同 VM 拓扑和相同负载下进行对照，本分支新增验证专用 feature 和配置 `rt-shared-wait-baseline`，用于重建 `dev` 原有的共享 vCPU 阻塞等待与广播唤醒语义。该配置仅用于 A/B，不是正式运行配置，也不承载客户机专用功能。正式配置启用 `rt-poll-idle`，在一对一绑定的独占 pCPU 上让 vCPU 跨 Guest WFI 保持可运行，同时保留 pending IRQ、目标 pCPU IPI 和硬件后备物理 IRQ 生命周期。两组均采用 Linux 2-vCPU + ArceOS 控制 Guest。

正式 300 次 A/B 中，共享等待基线空载完成 300/300；双压力场景完成第 1 次事务后，Linux 在 `smp_call_function_many_cond` / `kick_all_cpus_sync` 出现连续 RCU stall，整组在 600 秒边界超时，因此记录为 1/300、`FAIL`。这里的“1/300”表示预期 300 次应用层控制事务只完成 1 次，不是 1 轮测试通过，也不是优化效果百分比。失败证据归档在 `tmp/ai-rtos/results/rt-before-after-20260816-072749/baseline/`，其中 `variant.status.txt` 和压力原始日志保留了超时状态与调用栈。由于基线压力样本不完整，本报告不计算改进百分比。
...
| `run_axvisor_rt_before_after.sh [次数] [超时] [压力进程] [轮数]` | 默认 `300 360 2 1` | `rt-before-after-*/baseline`、`optimized`、状态与原始日志 | 相同 VM/负载下重建的 `dev` 共享等待基线与正式优化路径；超时按失败证据保留 |
...
`run_axvisor_realtime_matrix.sh` 默认使用 Guest 间私有 `hub`。`AICP_TRANSPORT=usernet` 只用于 hostfwd 兼容排障，不作为评分主数据。`run_axvisor_rt_before_after.sh` 的 `baseline` 使用本分支新增的验证专用配置 `qemu-aarch64-rt-shared-wait-baseline.toml`，重建 `dev` 原有共享等待语义；`optimized` 使用正式实时配置。两者使用相同请求次数、压力进程和 VM 拓扑。基线超时会写入 `variant.status.txt` 并使脚本失败；只有两组样本均完整时才计算改进百分比。
...
| 改造前后数据，5 分 | QEMU 已实测；基线压力长样本失败 | 同拓扑共享等待基线空载 300/300；双压力完成 1/300 后发生 Linux RCU stall 并在 600 秒超时。正式优化路径空载、双压力均为 300/300，并完成八组闭环回归 |
...
| 评分项 | 完成状态 | 证据 |
| --- | --- | --- |
| 工程完整性与文档，15 分 | 已完成 QEMU 范围交付 | 源码、配置、构建脚本、测试脚本、结果采集、复现命令和技术报告齐全 |
...
完整功能验收还包括八组双 Guest 闭环、三种原生 RTOS 周期基线、C 线协议 12/12、C 客户端事务 2/2、RTOS 服务状态机 1/1、八组网络隔离 PASS、RT-Thread 可靠性 8/8、StarryOS UDP 故障恢复 20/20、实时 A/B、ArceOS 10000/10000 双压力长稳和 Rust YOLOv8n CPU 3/3。RKNN/NPU、实体 GPIO/PWM/电机、物理网络和逻辑分析仪测量属于第七章列出的待上板验证项。
