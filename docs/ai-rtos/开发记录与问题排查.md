...
维护和排障时，“支持”需要结合系统角色与验证深度理解。启动成功、原生 RTOS 周期基线通过和 AI 网络控制闭环通过分别代表不同验证层级，问题定位与结果归档时应记录实际完成的层级和严格成功标记。
...
1. 通用配置仍使用每 VM 共享等待队列；本分支新增的验证 feature `rt-shared-wait-baseline` 精确保留最新 `dev` 的 `notify_all(false)` 阻塞唤醒语义。
...
本分支新增的 `os/axvisor/configs/board/qemu-aarch64-rt-shared-wait-baseline.toml` 启用最新 `dev` 原有的共享 vCPU 等待队列和 `notify_all(false)` 广播唤醒路径。正式 CPU 隔离配置启用 `rt-poll-idle`，让一对一绑定到独占 pCPU 的 vCPU 跨 Guest WFI 保持可运行；硬件后备 IRQ 仍走 pending、目标 pCPU IPI 和 Guest EOI 退休路径。设备 poll 通知只发布原子请求，不发送冗余 IPI。该策略只适用于文档声明的独占 pCPU 拓扑；共享 pCPU 实验应关闭 `rt-poll-idle`。`rt-shared-wait-baseline` 仅用于回归与 A/B，不是正式运行配置。
...
在最新 `dev` 基线上，共享阻塞等待路径的小样本能够完成；正式 300 次运行中，空载完成 300/300，双压力只完成 1/300，随后 Linux 出现连续 RCU stall 并在 600 秒边界超时。优化的 CPU 隔离轮询、pending IRQ 与目标 pCPU IPI 路径已完成空载和双压力各 300/300，以及 Linux/StarryOS 分别搭配四种控制 Guest 的八组 smoke 闭环。基线压力样本不完整，因此只报告失败现象、运行时长和原始调用栈，不换算延迟改善百分比。旧提交分支产生的三轮百分比属于旧架构历史数据，只用于问题演进排查。
...
### 7.35 迁移前实时候选版本的历史回归口径

迁移到最新 `dev` 之前的实时候选版本曾完成以下验证。这些结果用于回溯设计演进，不作为当前分支的评审数据：
